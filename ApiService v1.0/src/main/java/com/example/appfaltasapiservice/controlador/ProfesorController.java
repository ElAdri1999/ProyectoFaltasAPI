...
public class ProfesorController {
	...
	@PostMapping("/login")
	public ResponseEntity<?> validateUser(@RequestHeader("user") String usuario, @RequestHeader("pwd") String contra) {
		try {
			TypedQuery<Profesor> q = em.createQuery("SELECT p FROM Profesor p WHERE p.user = :user AND p.password = md5(:pwd)", Profesor.class);
			q.setParameter("user", usuario);
			q.setParameter("pwd", contra);
